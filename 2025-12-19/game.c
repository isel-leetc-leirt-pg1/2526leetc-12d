...
// Devolve um ponto com (row,col) onde foi feito o click
// no card (nas posições x,y do ecrã) ou 
// (-1,-1) caso seja fora de um card
Point getCardPosition(MouseEvent me){
  Point p_res = {-1, -1};

  if (me.button!=BUTTON_LEFT) return p_res; // não fez o click
  
  // Ver se o click foi feito dentro de um Card
   for (int row=0; row<N_COLS; row++)
     for (int col=0; col<N_COLS; col++){
        Rect cell_rect = board[row][col].rect;
        if (me.x>=cell_rect.p.x &&
            me.x<=cell_rect.p.x + cell_rect.sz.width &&
            me.y>=cell_rect.p.y &&
            me.y<=cell_rect.p.y + cell_rect.sz.height) {
              p_res.x = row;
              p_res.y = col;
              break;
            }
...
void my_mouse_handler(MouseEvent me){
   // printf("Mouse Event pos=(%d,%d) type=%d state=%d button=%d...\n", me.x, me.y, me.type, me.state, me.button);
  Point p = getCardPosition(me);
  if (p.x==-1) return ;
  printf("Card row=%d col=%d\n", p.x, p.y);
}
int main(){
   graph_init2("Card Board", WINDOW_WIDTH, WINDOW_HEIGHT); 
   graph_regist_timer_handler(clockHandler, 1000);

   graph_regist_mouse_handler(my_mouse_handler);
